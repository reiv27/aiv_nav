...
# x,y,theta,R_min,rho_0,min_dist,closest_lidar_x,closest_lidar_y,disk_x,disk_y
data = np.loadtxt(f'{sys.argv[1]}', delimiter=',')
print(data.shape)
n = data.shape[0]
t1 = int(sys.argv[2])
t2 = int(sys.argv[3])
...
# # plt.title('Debug Plot')
# plt.title('Real Telemetry. Part 3', fontsize=28)
...
plt.plot(np.arange(0, n, 1), dR, color='black')
plt.plot(np.arange(0, n, 1), 0.0*np.arange(0, n, 1), color='blue', linestyle='--')
plt.xlabel(r'$ticks$', fontsize=20)
plt.ylabel(r'$d_R(t), m$', fontsize=20)
# plt.title('Distance to equidistant', fontsize=28)
plt.title(fr'Distance to equidistant. $R = {R_min:.2f}$ m', fontsize=28)
plt.grid(True)
plt.tick_params(axis='both', which='major', labelsize=20)
stats_text = f'max:  {np.abs(np.max(dR[s:])):.3f}\n' \
             f'mean: {np.mean(dR[s:]):.3f}\n' \
             f'std:  {np.std(dR[s:]):.3f}'
plt.text(0.98, 0.98, stats_text, transform=plt.gca().transAxes,
